...
static func _fit_cubic_spline(polyline:PackedVector2Array, first:int, last:int, tangent_0:Vector2, tangent_1:Vector2, error:float) -> PackedVector2Array:
	...
	result.append_array(
		_fit_cubic_spline(polyline, first, split_index, tangent_0, -split_tangent, error)
	)
	result.append_array(
		_fit_cubic_spline(polyline, split_index, last, split_tangent, tangent_1, error)
	)
...
static func _fit_cubic_bezier(polyline:PackedVector2Array, first:int, last:int, t_values:Array[float], tangent_0:Vector2, tangent_1:Vector2) -> PackedVector2Array:
	var result : PackedVector2Array = [polyline[0], Vector2(), Vector2(), polyline[polyline.size() - 1]]

	# compute the A's
	var A : Array[PackedVector2Array]
	A.resize(polyline.size())
	
	for i:int in range(0, polyline.size()):
		var u : float = t_values[i]
		
		A[i].resize(2)
		A[i][0] = tangent_0 * 3 * pow(1 - u, 2) * u
		A[i][1] = tangent_1 * 3 * (1 - u) * pow(u, 2)
	...
	for i:int in range(0, polyline.size()):
		...
		var tmp : Vector2 = polyline[i] - result[0].bezier_interpolate(result[0], result[3], result[3], u)
...
static func _chord_length_parameterize(polyline:PackedVector2Array, first:int, last:int) -> Array[float]:
	...
	assert(t_values.size() == last - first)
...
static func _compute_max_error(
	...
	for i in range(0, polyline.size()):
		assert(false) # bezier_interpolate may not like this
		var p : Vector2 = bezier[0].bezier_interpolate(bezier[1], bezier[2], bezier[3], t_values[i])
